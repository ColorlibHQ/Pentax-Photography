...
// social media
if ( ! function_exists( 'pentax_social' ) ) {
	function pentax_social( $args = array()  ){
		...
		$url = pentax_opt('pentax_social_url');
...
function pentax_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'pentax' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'pentax' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'pentax' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function pentax_set_post_views($postID) {
    $count_key = 'pentax_post_views_count';
...
// blog post categoty 
function pentax_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'pentax' ) );
