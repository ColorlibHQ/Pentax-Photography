 ...
 */
	...
	final class Pentax{
		...
		private function setup(){
			...
			$enqueu = new pentax_Enqueue();
			$enqueu->scripts = $this->enqueue() ;
			$enqueu->pentax_scripts_enqueue_init() ;

		}
		// Theme Support
		public function support(){
			// content width
	        $GLOBALS['content_width'] = apply_filters( 'pentax_content_width', 751 );

	        
	        // text domain for translation.
	        load_theme_textdomain( 'pentax', PENTAX_DIR_PATH . '/languages' );
			...
			add_image_size( 'pentax_widget_post_thumb', 80, 80, true );
	        ...
	        register_nav_menus( array(
	            'primary-menu'   => esc_html__( 'Primary Menu', 'pentax' ),
	            'social-menu'    => esc_html__( 'Social Menu', 'pentax' ),
	        ) );
		...
		private function enqueue(){
			...
			$cssPath = PENTAX_DIR_CSS_URI;
			$jsPath  = PENTAX_DIR_JS_URI;
		...
		private function google_font(){
			...
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'pentax' ) ) {
		...
		private function customizer_init(){
			...
			// Instantiate pentax theme customizer
			$pentax_theme_customizer = new pentax_theme_customizer();
		}
		...
		public function epsilon_quickie(){
				...
				'links' => array(
					array(
						'link_to'   => 'pentax_theme_options_panel',
